    ...
    if not os.path.exists(csv_path):
        if os.path.exists(zip_path):
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                ...
                print("CF_data2.csv extracted successfully.")
        else:
...
def recommend_cf(user_id, book_name):
...
def hybrid_recommend(user_id, book_name):
    ...
    # Ensure scores are float, not lists
...
# ------------- Route handler functions (no decorators) -------------------
