...
#### Data Constraints
- Sparse user-item matrix (cold start problem)
- Highly skewed rating distribution (few heavy users, many light users)
- Metadata limitations (title + author only → weak semantic richness)
- No real-time streaming updates (batch-based recommendation)

#### Compute Constraints
- TF-IDF + cosine similarity → O(N²) similarity matrix risk
- CF user-user similarity → memory-heavy for large user base
- FastAPI single-instance deployment on Hugging Face → limited CPU/RAM

#### API Constraints
- Latency must remain < 500ms for user experience
- Stateless endpoints → no session-level personalization
- No caching layer currently implemented

### 2. Failure Modes Considered
#### Cold Start Failure
- New user with < threshold ratings → CF fails
- Fallback strategy:
  >If user inactive → switch to Popularity-Based
  >If metadata exists → use CBF only
#### Expert Matching Bias
##### Problem:
Expert users (>300 ratings) can dominate CF similarity space.

##### Failure risk:
- Overfitting toward niche heavy readers
- Reduced diversity in recommendations

##### Mitigation:
- Normalize rating vectors
- Weight similarity by user overlap count
- Cap influence of extremely high-volume raters

#### Popularity Bias
Popularity model can:
- Reinforce already famous books
- Reduce catalog exposure
Mitigation:
- Minimum rating count threshold
- Combine with CBF in hybrid

#### Latency Explosion
Computing similarity on-the-fly at scale will:
- Increase API latency
- Crash under concurrent load

Mitigation:
- Precompute similarity matrices
- Cache top-N recommendations per user
- Move to async architecture if scaling

### 3. Hybrid Model Strategy (With System Thinking)
Current:
Fixed weights
CBF = 0.4
CF = 0.6
...
Dynamically adjust weight based on:
- User activity level
- Confidence score of similarity
- Number of co-rated items
>Example:
>New user → CBF = 0.8
>Active user → CF = 0.7

Sparse similarity → fallback to popularity
### 4. Scaling Strategy
#### Stage 1 – Current (Small Scale)
- Threshold-based CF
- Precompute similarities
- Cache recommendations
#### Stage 2 – Mid Scale
- Store user embeddings (instead of raw similarity)
- Update embedding incrementally when new book added
- ANN search for neighbors
#### Stage 3 – Large Scale
- Separate training service
- Batch retraining nightly
- Real-time feature store for last-N behavior
